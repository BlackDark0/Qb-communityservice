local Keys = {
  ...
  AddEventHandler('QBCore:Client:OnPlayerLoaded', function()
	  TriggerServerEvent('qb-communityservice:checkIfSentenced') -- Check if the user is sentenced when the player is loaded.
  end)
  ...
  function DisableViolentActions()
	  ...
	  DisablePlayerFiring(playerPed, true) -- Disable firing all together if they somehow bypass inzone Mouse Disable
	  ...
	  if IsDisabledControlJustPressed(2, 37) then -- If Tab is pressed, send error message
		  SetCurrentPedWeapon(playerPed, GetHashKey("WEAPON_UNARMED"), true) -- If tab is pressed, it will set them to unarmed
	  end
