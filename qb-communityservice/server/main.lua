...
QBCore.Commands.Add("servcom", _U('give_player_community'), {{name = "id", help = _U('target_id')}, {name = "actions", help = _U('action_count_suggested')}}, false, function(source, args, user)
    local Player = QBCore.Functions.GetPlayer(source)
    
    if args[1] and tonumber(args[1]) and tonumber(args[2]) then
        ...
        if TargetPlayer then
            ...
            TriggerEvent('qb-communityservice:sendToCommunityService', tonumber(args[1]), actions)
            local sentenceMessage = playerName .. " ha sido sentenciado a " .. actions .. " acción/es de servicio comunitario."
            TriggerClientEvent('chat:addMessage', -1, { args = { _U('system_msn'), sentenceMessage } })
            TriggerClientEvent('chat:addMessage', TargetPlayer.PlayerData.source, { args = { _U('system_msn'), "Has sido sentenciado a " .. actions .. " acción/es de servicio comunitario." } })
            TriggerClientEvent('chat:addMessage', source, { args = { _U('system_msn'), "Has sentenciado a " .. playerName .. " a " .. actions .. " acción/es de servicio comunitario." } })
        else
...
QBCore.Commands.Add("finservcom", "Finalizar un jugador los servicios comunitarios que tenía/tiene", { { name = "id", help = _U('target_id') } }, false, function(source, args, user)
    local Player = QBCore.Functions.GetPlayer(source)
        ...
        if result[1] then
            MySQL.Async.fetchAll('UPDATE communityservice SET actions_remaining = actions_remaining - 1 WHERE identifier = @identifier', {
                ...
                MySQL.Async.fetchAll('SELECT actions_remaining FROM communityservice WHERE identifier = @identifier', {
                    ...
                    local remainingActions = newResult[1].actions_remaining
                    if remainingActions > 0 then
                        TriggerClientEvent('qb-communityservice:updateServiceStatus', _source, remainingActions)
                    else
                        TriggerClientEvent('qb-communityservice:updateServiceStatus', _source, 0)
                    end
                end)
            end)
        else
    ...
    MySQL.Async.fetchAll('SELECT * FROM communityservice WHERE identifier = @identifier', {
        ...
        if result[1] then
            MySQL.Async.fetchAll('UPDATE communityservice SET actions_remaining = actions_remaining + @extension_value WHERE identifier = @identifier', {
                ...
                local newActions = result[1].actions_remaining + Config.ServiceExtensionOnEscape
                TriggerClientEvent('qb-communityservice:updateServiceStatus', _source, newActions)
            end)
        else
            print ("qb-communityservice :: Problem matching player identifier in database to reduce actions.")
        end
